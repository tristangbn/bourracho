...
def sample_message(sample_users):
    return Message(id="m1", content="Hello", timestamp=datetime.now(), issuer=sample_users[0])


def test_registry_init(temp_dir):
    conv_reg = ConversationsRegistry(conversations_registry_id="coucou", persistence_dir=temp_dir)
    assert conv_reg.conversation_stores == {}
    assert conv_reg.persistence_dir == temp_dir
...
def test_add_user(temp_dir, sample_metadata, sample_users):
    ...
    conv_reg.add_conversation(sample_metadata)
    conv_reg.add_user(sample_users[1], sample_metadata.id)
...
def test_errors_on_invalid_conversation(temp_dir, sample_message, sample_users):
    ...
    with pytest.raises(ValueError):
        conv_reg.add_user(sample_users[0], "bad_id")
    with pytest.raises(ValueError):
